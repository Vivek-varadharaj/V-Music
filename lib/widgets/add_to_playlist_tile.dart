 ...
 final String playlistName;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){
        db.addToPlaylist(audioModel: audioModel,playlistName: playlistName,context: context);
            ...
            children: [
                Expanded(
                  ...
                  child: Padding(
                    ...
                    child: Text(playlistName,style: StyleForApp.heading,),
                  )),
               Padding(
                 padding: const EdgeInsets.only(right:15.0),
                 child: Container(child: (playlistName!= "Favorites") ? Icon(FontAwesomeIcons.arrowAltCircleRight,color: Colors.white ,) : Icon(Icons.favorite_border_rounded,color: Colors.red,)),
               ),
            ],
 ...
 playlistSongs= await db.getSongs(playlistName);
  ...
  bool isExists(){
    if(playlistSongs!.length!=0){
      final List <AudioModel> songs = playlistSongs!.where((element) => element.id==myAudioModelSong!.id).toList();
    if(songs.length == 0){
      return false;
    }
  else return true;
